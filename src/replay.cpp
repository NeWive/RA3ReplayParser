...
unsigned int ReplayParser::readUInt32LittleEndian(const unsigned char in[]) {
    return ((unsigned int) ((in)[0] | ((in)[1] << 8) | ((in)[2] << 16) | ((in)[3] << 24)));
}

unsigned int ReplayParser::readUInt16LittleEndian(const unsigned char in0, const unsigned char in1) {
    return (((unsigned int) (in1) << 8) | ((unsigned int) (in0)));
}

std::string ReplayParser::read2ByteString(std::istream &in, unsigned int separator) {
    CodePoint ccp;
    TBString cbuf;
    std::string s;
    while (true) {
...
std::string ReplayParser::read2ByteStringN(std::istream &in, unsigned int separator, size_t len) {
    CodePoint ccp;
    TBString cbuf;
    std::string s;
    size_t n = len;
...
std::vector<std::string> ReplayParser::tokenize(const std::string &str, const std::string &delimiters) {
...
std::string ReplayParser::getFaction(unsigned int f) {
    switch(f)
    {
        case 1:  return "天眼帝国";
...
std::string ReplayParser::genFailedMessage(std::string message) {
    jsoncons::json out;
...
std::string ReplayParser::parseReplayFile(char f[]) {
    // replay头信息
    ReplayParser::ReplayHeader* header = (ReplayParser::ReplayHeader *) malloc(sizeof (ReplayParser::ReplayHeader));
